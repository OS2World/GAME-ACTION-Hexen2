...
void P_LoadVertexes (int lump)
{
	...
	{
		li->x = SHORT_M(ml->x)<<FRACBITS;
		li->y = SHORT_M(ml->y)<<FRACBITS;
	}
...
void P_LoadSegs (int lump)
{
	...
	{
		li->v1 = &vertexes[SHORT_M(ml->v1)];
		li->v2 = &vertexes[SHORT_M(ml->v2)];

		li->angle = (SHORT_M(ml->angle))<<16;
		li->offset = (SHORT_M(ml->offset))<<16;
		linedef = SHORT_M(ml->linedef);
		ldef = &lines[linedef];
		li->linedef = ldef;
		side = SHORT_M(ml->side);
...
void P_LoadSubsectors (int lump)
{
	...
	{
		ss->numlines = SHORT_M(ms->numsegs);
		ss->firstline = SHORT_M(ms->firstseg);
	}
...
void P_LoadSectors (int lump)
{
	...
	{
		ss->floorheight = SHORT_M(ms->floorheight)<<FRACBITS;
		ss->ceilingheight = SHORT_M(ms->ceilingheight)<<FRACBITS;
		ss->floorpic = R_FlatNumForName(ms->floorpic);
		ss->ceilingpic = R_FlatNumForName(ms->ceilingpic);
		ss->lightlevel = SHORT_M(ms->lightlevel);
		ss->special = SHORT_M(ms->special);
		ss->tag = SHORT_M(ms->tag);
		ss->thinglist = NULL;
...
void P_LoadNodes (int lump)
{
	...
	{
		no->x = SHORT_M(mn->x)<<FRACBITS;
		no->y = SHORT_M(mn->y)<<FRACBITS;
		no->dx = SHORT_M(mn->dx)<<FRACBITS;
		no->dy = SHORT_M(mn->dy)<<FRACBITS;
		for (j=0 ; j<2 ; j++)
		{
			no->children[j] = SHORT_M(mn->children[j]);
			for (k=0 ; k<4 ; k++)
				no->bbox[j][k] = SHORT_M(mn->bbox[j][k])<<FRACBITS;
		}
	}
	Z_Free (data);
}
...
void P_LoadThings(int lump)
{
	...
	{
		mt->tid = SHORT_M(mt->tid);
		mt->x = SHORT_M(mt->x);
		mt->y = SHORT_M(mt->y);
		mt->height = SHORT_M(mt->height);
		mt->angle = SHORT_M(mt->angle);
		mt->type = SHORT_M(mt->type);
		mt->options = SHORT_M(mt->options);
		P_SpawnMapThing(mt);
	}
	P_CreateTIDList();
...
void P_LoadLineDefs(int lump)
{
	...
	{
		ld->flags = SHORT_M(mld->flags);

		// Old line special info ...
		//ld->special = SHORT_M(mld->special);
		//ld->tag = SHORT_M(mld->tag);
		...
		v1 = ld->v1 = &vertexes[SHORT_M(mld->v1)];
		v2 = ld->v2 = &vertexes[SHORT_M(mld->v2)];
		ld->dx = v2->x - v1->x;
		ld->dy = v2->y - v1->y;
		if (!ld->dx)
		...
		ld->sidenum[0] = SHORT_M(mld->sidenum[0]);
		ld->sidenum[1] = SHORT_M(mld->sidenum[1]);
		if (ld->sidenum[0] != -1)
...
void P_LoadSideDefs (int lump)
{
	...
	{
		sd->textureoffset = SHORT_M(msd->textureoffset)<<FRACBITS;
		sd->rowoffset = SHORT_M(msd->rowoffset)<<FRACBITS;
		sd->toptexture = R_TextureNumForName(msd->toptexture);
		sd->bottomtexture = R_TextureNumForName(msd->bottomtexture);
		sd->midtexture = R_TextureNumForName(msd->midtexture);
		sd->sector = &sectors[SHORT_M(msd->sector)];
	}
	if(DevMaps)
	{
		W_UseAuxiliary();
...
void P_LoadBlockMap (int lump)
{
	...
	for (i=0 ; i<count ; i++)
		blockmaplump[i] = SHORT_M(blockmaplump[i]);
...
void P_SetupLevel(int episode, int map, int playermask, skill_t skill)
{
   ...
   #ifdef __OS2_DEMO_VERSION_JUL__
   if (lumpnum == -1)   //Not found
   {
      return;
   }
   #endif
