...
void V_DrawPatch(int x, int y, patch_t *patch)
{
	...
	y -= SHORT_M(patch->topoffset);
	x -= SHORT_M(patch->leftoffset);
	if(x < 0 || x+SHORT_M(patch->width) > SCREENWIDTH || y < 0
		|| y+SHORT_M(patch->height) > SCREENHEIGHT)
	{
      #ifdef __OS2_DEMO_VERSION_JUL__
      ST_Message ("Bad V_DrawPatch. Assume demo version");
      return;
      #else
	...
	w = SHORT_M(patch->width);
	for(; col < w; x++, col++, desttop++)
	{
		column = (column_t *)((byte *)patch+LONG_M(patch->columnofs[col]));
...
void V_DrawFuzzPatch (int x, int y, patch_t *patch)
{
	...
	if (x<0||x+SHORT_M(patch->width) >SCREENWIDTH || y<0 || y+SHORT_M(patch->height)>SCREENHEIGHT)
		I_Error ("Bad V_DrawPatch");
