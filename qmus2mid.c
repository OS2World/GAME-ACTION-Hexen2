             ...
             int2 division, int BufferSize, int nocomp )
{
  ...
  if( !nodisplay )
  {
      ...
      strcpy (buff, mus);
      strcat (buff, "(");
      sprintf(buff, "%s%lu", buff, (unsigned long) file_data.st_size);
      strcat (buff, " bytes) contains ");
      sprintf(buff, "%s%d", buff, MUSh.channels);
      strcat (buff, " melodic channel(s)");
      //sprintf (buff, "%s (%lu bytes) contains %d melodic channel%s.", mus,
  ...
  if( BufferSize )
    {
      ...
      if( !nodisplay )
        ST_Message ("Track buffer size set to %d KB.\n", BufferSize ) ;
    }
  
  if( !nodisplay )
    {
      ST_Message ( "Converting..." ) ;
      fflush( stdout ) ;
    }
  event = getc( file_mus ) ;
  et = event_type( event ) ;
  ...
  if( !nodisplay ) ST_Message ( "done" ) ;
  if( ouch )
    ST_Message ( "WARNING : There are bytes missing at the end of %s. The end of the MIDI file might not fit the original one.", mus );
  if( !division )
    division = 89 ;
  else
    if( !nodisplay ) ST_Message ( "Ticks per quarter note set to %d.", division ) ;
  if( !nodisplay )
    {
      if( division != 89 )
        {
          ...
          ST_Message ("Playing time of the MUS file : %u'%.2u''.", min, sec ) ;
        }
      ...
      if( division != 89 )
        ...
        ST_Message ( "Playing time" ) ;
      ST_Message ( " : %u'%.2u''.", min, sec ) ;
    }
  if( !nodisplay )
    {
      ST_Message ( "Writing..." ) ;
            ...
            int size, int nocomp, int *ow )
{
  ...
  if ( !*ow ) {
    ...
    if ( file ) {
      fclose(file);
      ST_Message ( "qmus2mid: file %s exists, not removed.", mid ) ;
      return 2 ;
    }
  }
  ...
  if( error )
    {
      ...
      switch( error )
        {
        case NOTMUSFILE :
          ST_Message ("%s is not a MUS file.", mus ) ; break ;
        case COMUSFILE :
          ST_Message ("Can't open %s for read.", mus ) ; break ;
        case COTMPFILE :
          ST_Message ("Can't open temp file." ) ; break  ;
        case CWMIDFILE :
          ST_Message ("Can't write %s (?).", mid ) ; break ;
        case MUSFILECOR :
          ST_Message ("%s is corrupted.", mus ) ; break ;
        case TOOMCHAN :
          ST_Message ("%s contains more than 16 channels.", mus ) ; break ;
        case MEMALLOC :
          ST_Message ("Not enough memory." ) ; break ;
        default : break ;
        }
      return 4 ;
    }
  ...
  if( !nodisplay )
    {
      ST_Message ( "%s converted successfully.", mus ) ;
      ...
      ST_Message ( "%s (%scompressed) written%s.", mid, nocomp ? "NOT " : "",
             file ? buffer : ""  ) ;
    }
...
int main( int argc, char *argv[] )
{
  ...
  if ( !LittleEndian() ) {
    ST_Message ("Sorry, this program presently only works on little-endian machines...");
    exit( EXIT_FAILURE ) ;
  }
