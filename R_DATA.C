	...
	for (i=0 , patch = texture->patches; i<texture->patchcount ; i++, patch++)
	{
		...
		for ( ; x<x2 ; x++)
		{
			...
			patchcol = (column_t *)((byte *)realpatch + 
				LONG_M(realpatch->columnofs[x-x1]));
...
//
	...
	for (i=0 , patch = texture->patches; i<texture->patchcount ; i++, patch++)
	{
		...
		for ( ; x<x2 ; x++)
		{
			...
			colofs[x] = LONG_M(realpatch->columnofs[x-x1])+3;
		}
	}
...
//
	...
	nummappatches = LONG_M ( *((int *)names) );
	name_p = names+4;
...
//
	...
	for (i=0 ; i<numtextures ; i++, directory++)
	{
		...
		offset = LONG_M(*directory);
		if (offset > maxoff)
			I_Error ("R_InitTextures: bad texture directory");
		mtexture = (maptexture_t *) ( (byte *)maptex + offset);
		texture = textures[i] = Z_Malloc (sizeof(texture_t) 
			+ sizeof(texpatch_t)*(SHORT_M(mtexture->patchcount)-1), PU_STATIC,
			0);
		texture->width = SHORT_M(mtexture->width);
		texture->height = SHORT_M(mtexture->height);
		texture->patchcount = SHORT_M(mtexture->patchcount);
		memcpy (texture->name, mtexture->name, sizeof(texture->name));
		mpatch = &mtexture->patches[0];
		patch = &texture->patches[0];
		for (j=0 ; j<texture->patchcount ; j++, mpatch++, patch++)
		{
			patch->originx = SHORT_M(mpatch->originx);
			patch->originy = SHORT_M(mpatch->originy);
			patch->patch = patchlookup[SHORT_M(mpatch->patch)];
			if (patch->patch == -1)
				I_Error (
...
void R_InitSpriteLumps (void)
{
	...
	for (i=0 ; i< numspritelumps ; i++)
	{
		...
		spritewidth[i] = SHORT_M(patch->width)<<FRACBITS;
		spriteoffset[i] = SHORT_M(patch->leftoffset)<<FRACBITS;
		spritetopoffset[i] = SHORT_M(patch->topoffset)<<FRACBITS;
	}
}
...
int	R_TextureNumForName (char *name)
{
	...
	if (i == -1)
   {
      ...
      ST_Message("W_GetNumForName: %s not found!  Assuming shareware wad.", name);
      return 1;
#else
