...

{
   if (ulFlags & MIX_STREAM_ERROR)
   {
      ...
      if (ulStatus == ERROR_DEVICE_UNDERRUN)
         strcpy (buff, "ERROR_DEVICE_UNDERRUN");
      else if (ulStatus == ERROR_DEVICE_OVERRUN)
         strcpy (buff, "ERROR_DEVICE_OVERRUN");
      else
         buff[0] = 0;

      ST_Message ("DartEvent: MIX_STREAM_ERROR %s", buff);
   }
   else if (ulFlags == MIX_WRITE_COMPLETE)
   {
      ST_Message ("DartEvent: MIX_WRITE_COMPLETE");
      PlayDART( pmData);
         // create next buffer
      I_UpdateSound();
   }
   else
      ST_Message ("DartEvent: ulFlags=%d", (int) ulFlags);
   return( TRUE );
}
...
void  MciError (ULONG ulError)
{
   ...
   if (ulError != 5010)
      rc = mciGetErrorString( ulError, (PSZ)&szBuffer[0], 256);

   ST_Message ("MciError#%d:%s", ulError, szBuffer);
}
...
{
   static MCI_AMP_OPEN_PARMS   AmpOpenParms;
   int   i;//, rc;
   ULONG rc;   //JUL : Fuck, get the right type!!!
   ...
   ST_Message ("audio handle=%x", dartData->usDartID);
   ...
   ST_Message ("InitDART: #%d buffers of length %d",
...
int  RegisterMIDI( PWINDATA midiData, void* data, int len)
{
   ...
   blah = fopen( "hexen.mus", "wb");
   fwrite( data, len, 1, blah);
   fclose( blah);
   qmus2mid( "hexen.mus", "hexen.mid", 0, 89,64,1);
...
void OpenMIDI( PWINDATA midiData)
{
   ...
   mop.pszElementName = (PSZ)"hexen.mid";
...
void    SetMIDIVolume( PWINDATA midiData, int vol)
{
   ...
   ST_Message ("SetMIDIVolume");
   midiData->midiVolume = vol;
   msp.ulLevel = vol * 100 / 15;
