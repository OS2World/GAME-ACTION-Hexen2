...
void M_FindResponseFile(void)
{
	...
	for(i = 1; i < myargc; i++)
	{
		if(myargv[i][0] == '@')
		{
            ...
            ST_Message("No such response file!");
            exitHexenOS2();
            #else
...
void WritePCXfile (char *filename, byte *data, int width, int height, byte *palette)
{
	...
	pcx->xmin = 0;
	pcx->ymin = 0;
	pcx->xmax = SHORT_M(width-1);
	pcx->ymax = SHORT_M(height-1);
	pcx->hres = SHORT_M(width);
	pcx->vres = SHORT_M(height);
	memset (pcx->palette,0,sizeof(pcx->palette));
	pcx->color_planes = 1;      // chunky image
	pcx->bytes_per_line = SHORT_M(width);
	pcx->palette_type = SHORT_M(2);       // not a grey scale
