...
void W_AddFile(char *filename)
{
	...
	{ // Single lump file
		...
		singleinfo.size = LONG_M(filelength(handle));
	...
	{ // WAD file
		...
		header.numlumps = LONG_M(header.numlumps);
		header.infotableofs = LONG_M(header.infotableofs);
...
//		fileinfo = alloca(length);
	...
	for(i = startlump; i < numlumps; i++, lump_p++, fileinfo++)
	{
		...
		lump_p->position = LONG_M(fileinfo->filepos);
		lump_p->size = LONG_M(fileinfo->size);
...
void W_OpenAuxiliary(char *filename)
{
	...
	for(i = 0; i < numlumps; i++, destLump++, sourceLump++)
	{
		...
		destLump->position = LONG_M(sourceLump->filepos);
		destLump->size = LONG_M(sourceLump->size);
...
int	W_GetNumForName (char *name)
{
   ...
   ST_Message ("R_TextureNumForName: %s not found!  Assuming shareware wad.", name);
#else
