...
void ST_UpdateNotches(int notchPosition)
{
	...
	int y = ST_PROGRESS_Y;
	SlamBlockHR(x,y, ST_NOTCH_WIDTH,ST_NOTCH_HEIGHT, (char*)notchTable);
#endif
}
...
void ST_UpdateNetNotches(int notchPosition)
{
	...
	SlamBlockHR(x,y, ST_NETNOTCH_WIDTH, ST_NETNOTCH_HEIGHT, (char*)netnotchTable);
#endif
}
...
void ST_Message(char *message, ...)
{
	...
	{
   #ifdef __OS2_VERSION_JUL__
      WinMessageBox (HWND_DESKTOP, HWND_DESKTOP, (PSZ)buffer, (PSZ)"Debug",
